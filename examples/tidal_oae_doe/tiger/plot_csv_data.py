...
# Inputs
csvFileName = "outputs/100kt_50x_2-6MW_5-1-50m_Cook_Inlet_1M_rca_tiger_no2nd.csv"
...
for j in range(len(capacities)):
    plt.plot(np.multiply(radii,2), lcoe_results[:, j], label = f"{capacities[j]/1000} MW", marker='none')
...
for j in range(len(capacities)):
    plt.plot(np.multiply(radii,2), becc_results[:, j], label = f"{capacities[j]/1000} MW", marker='none')
plt.xlabel("Rotor Diameter (m)")
