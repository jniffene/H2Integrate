...
turbine = "modRM1"
baseCSVfile = "../tiger/outputs/3MW_modRM1.csv"
sensCSVfile = "outputs/hybrid_1cap_modRM1_100MW_wind.csv"
turbineName = "150 MW Hybrid Arrays: 1.5 MW Wind & 3 MW RM1"
...
windCSVfile_CookInlet = "../wind_only/outputs/100kt_100x_1,5MW_wind_Cook_Inlet_1M_rca.csv"
windVals = ['finance_subgroup_electricity.LCOE (USD/kW/h)', 'oae.carbon_credit_value (USD/t)']
df_wind_CI = pd.read_csv(windCSVfile_CookInlet, usecols=windVals)
r_start = 10
r_end = 30
r_step = 1
rotor_radii = np.arange(r_start, r_end+r_step, r_step)
keyVals_wind_CookInlet = df_wind_CI.to_numpy()[0] * np.ones([len(rotor_radii),2])
...
colors = cmap(np.linspace(0.4, 0.4, num_colors))
...
for k in range(len(keyResults)):
    for j in range(len(sens_vars)):
        ...
        plt.legend(["    0 MW Wind | 150 MW Tidal", "  45 MW Wind | 105 MW Tidal", "  75 MW Wind |   75 MW Tidal", "105 MW Wind |   45 MW Tidal"], loc='upper right')
        plt.minorticks_on()
        ...
        # plt.show()
        plt.savefig(f"{turbine}_{keyResults[k]}_hybrid_30-50-70_wind_1cap.png", dpi = 300, bbox_inches='tight')
        plt.close()
