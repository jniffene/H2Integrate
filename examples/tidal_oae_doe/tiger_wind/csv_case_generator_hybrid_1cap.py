...
vals = np.ones((len(rotor_radii) * len(windFracs), len(vars)))
counter = 0
...
varsDF.to_csv('hybrid_input_cases_1cap.csv', index=False, header=False)
valsDF = pd.DataFrame(vals)
valsDF.to_csv('hybrid_input_cases_1cap.csv', index=False, header=False, mode ='a')