...
windCSVfile_CookInlet = "../wind_only/outputs/100kt_50x_1,5MW_wind_Cook_Inlet_1M_rca.csv"
windCSVfile_Toksook_Bay = "../wind_only/outputs/100kt_50x_1,5MW_wind_Toksook_Bay_1M_rca.csv"
...
df_wind_TSB = pd.read_csv(windCSVfile_Toksook_Bay, usecols=windVals)
r_start = 10
r_end = 30
r_step = 1
rotor_radii = np.arange(r_start, r_end+r_step, r_step)
keyVals_wind_CookInlet = df_wind_CI.to_numpy()[0] * np.ones([len(rotor_radii),2])
keyVals_wind_ToksookBay = df_wind_TSB.to_numpy()[0] * np.ones([len(rotor_radii),2])
...
for i in range(len(capsLegend)):
    capsLegend[i] = str(round(baseDict["capacities"][i]/1000,1)) +" MW Tidal"
